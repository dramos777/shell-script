...
action=$(dialog --title "Gerenciador de Usuários 2.0" \
    ...
    case $action in
        Listar) ListUsers ;;
        Inserir)
        last_id="$(egrep -v "^#|^$" $DB | sort | tail -n 1 | cut -d $SEP -f 1)"
        next_id=$(($last_id+1))

        name=$(dialog --title "Cadastro de usuários" --stdout --inputbox "Digite seu nome" 0 0)
        [ ! "$name" ] && exit 1
        ...
        email=$(dialog --title "Cadastro de usuários" --stdout --inputbox "Digite seu E-mail" 0 0)
        [ $? -ne 0 ] && continue
        ...
        Remover) 
        users=$(egrep "^$|^#" -v "$DB" | sort -h | cut -d $SEP -f 1,2 | sed 's/:/ "/;s/$/"/')
        user_id=$(eval dialog --stdout --menu \"Escolha um usuário:\" 0 0 0 $users)
        [ $? -ne 0 ] && continue
